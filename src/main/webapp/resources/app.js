var app = angular.module('app', [ 'angularModalService', 'ngAnimate', 'ngRoute', 'cgBusy' ]);
...
app.controller('routerCtrl', function($scope, $http, $window, $q, $location) {
	...
	$scope.myhttp = function(conf) {
		...
		// Essa é a ID de teste no Mac OSX
		//var editorExtensionId = "badeikkolbajgcpbpcjboemhappeifmc";
...
app.controller('ctrl2', function($scope, $http, $interval, $window) {
	...
	$scope.versionAssijusChromeExtension = document.getElementById("chrome-extension-active").value;
	$scope.versionAssijusNativeClient = document.getElementById("native-client-active").value;
	if ($scope.versionAssijusChromeExtension != "0" && $scope.versionAssijusNativeClient == "0") {
		$scope.myhttp({
...
app.controller('ctrl', function($scope, $http, $interval, $window, $location, $filter, $timeout, ModalService) {
	...
	$scope.reportErrorAndResume = function(codigo, context, response) {
		...
		if (response.statusText)
			msg = "Erro " + context + ': ' + response.statusText;
		if (response.data.hasOwnProperty("errormsg") && response.statusText === undefined)
	...
	$scope.assinarDocumento = function(id) {
		...
		$scope.progress.start("Processando Assinatura Digital", 6 + 6);
	...
	$scope.assinarDocumentos = function(progress) {
		...
		progress.start($scope.PROCESSING, $scope.operacoes.length * 6 + 6);
	...
	$scope.gravarAssinatura = function(state, progress) {
		...
		$http({
			...
			if (!progress.active && $scope.endpoint && $scope.endpoint.usecallback && $scope.endpoint.callback) {
	...
	// 2 steps
	$scope.selecionarCertificado = function(progress) {
		...
		$scope.myhttp({
			...
			if ($scope.p11 && data && data.list) {
				$scope.showDialogForCerts(data.list);
				return;
			}
			...
			$scope.setCert(data);
			$scope.validarAuthKey(progress, $scope.list);
		}, function errorCallback(response) {
			delete $scope.documentos;
			progress.stop();
			if (response.data && response.data.errormsg.indexOf("CKR_") !== -1) {
	...
	$scope.prosseguirComPIN = function(userPIN) {
		if ((userPIN || "") == "") {
	...
	$scope.showDialogForPIN = function(errormsg) {
		ModalService.showModal({
			...
			modal.close.then(function(result) {
				$scope.prosseguirComPIN(result.pin);
			});
		});
	};
	...
	$scope.showDialogForCerts = function(list) {
		ModalService.showModal({
			...
			modal.element.modal();
			modal.close.then(function(result) {
				$scope.prosseguirComCertificado(result.subject);
			});
		});
	};

	// 3 steps
	$scope.buscarCertificadoCorrente = function(progress) {
		...
		$scope.myhttp({
			...
			if (data.hasOwnProperty('subject')) {
				...
				if ($scope.p11 && !$scope.hasOwnProperty('userPIN')) {
					$scope.showDialogForPIN();
					progress.stop();
					return;
				}
				$scope.selecionarCertificado(progress);
			}
		}, function errorCallback(response) {
			delete $scope.documentos;
			progress.stop();
	...
	$scope.testarSigner = function(progress) {
		...
		$scope.myhttp({
			...
			if (response.data.status == "OK") {
				$scope.p11 = response.data.provider.indexOf("PKCS#11") !== -1;
				document.getElementById("native-client-active").value = response.data.version;
				$scope.buscarCertificadoCorrente(progress);
			} else {
				progress.stop();
...
app.controller('PINController', function($scope, $element, $timeout, title, errormsg, close) {
...
app.controller('CertsController', function($scope, $element, $timeout, title, list, close) {
